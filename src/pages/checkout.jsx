      ...
      {cartItems.length === 0 ? (
        ...
        <div>
          {cartItems.map((item) => (
            <div
              key={item.id}
              className="grid grid-cols-3 mt-3 w-64 pb-3 border-b-2"
            >
              <div className="">
                <img
                  src={item.image.url}
                  alt={item.title}
                  className="object-cover mt-6 size-24"
                />
              </div>
          ...
          <div className="grid grid-cols-3 mt-3 w-64">
            <div className=""></div>
            <h3 className="col-span-2 ms-4  font-bold">
              Total: {calculateTotal()} NOK
            </h3>
            <div className=""></div>
            <Link
              to="/checkoutSuccess"
              onClick={() => clearCart()}
              className="col-span-2 mt-8 mb-8 w-full rounded-none text-center bg-red-600 text-white px-2.5 py-1.5 text-sm font-semibold shadow-sm ring-1 ring-inset hover:bg-red-500"
            >
              Checkout
            </Link>
          </div>
